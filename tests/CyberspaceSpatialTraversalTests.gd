...
func _test_deterministic_layered_network() -> void:
	...
	_expect(initial.anchors[&"A"] == Vector3(0, 0, 0) and initial.anchors[&"B"] == Vector3(-5, 0, -12) and initial.anchors[&"C"] == Vector3(5, 0, -12), "entry and first layer use the exact centered lane formula")
	_expect(initial.anchors[&"D"] == Vector3(-10, 0, -24) and initial.anchors[&"E"] == Vector3(0, 0, -24) and initial.anchors[&"F"] == Vector3(10, 0, -24), "second layer uses fixed ten-unit lanes and twelve-unit depth")
	_expect(initial.anchors[&"G"] == Vector3(0, 0, -36) and initial.anchors[&"H"] == Vector3(0, 0, -48), "deep single-node layers remain centered at deterministic forward coordinates")
