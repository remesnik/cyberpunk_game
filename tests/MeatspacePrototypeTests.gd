...
func _ready() -> void:
	...
	for time in ["DAY", "DUSK", "NIGHT", "DAWN"]:
		...
		_expect(not exterior.sequencer.eligible_event_ids().is_empty(), "%s selects authored one-shot city events" % time)
		...
		_expect(exterior.layers.size() == 3 and is_equal_approx((exterior.layers.rooftop_wind as AudioStreamPlayer).volume_db, float(profile.layers_db.rooftop_wind)), "%s applies its layered exterior mix" % time)
	...
	_expect((room.dressing[&"CHAIR_JACKET"].node as Node3D).visible and (room.dressing[&"DESK_CUP"].node as Node3D).visible, "deck state adds authored lived-in dressing")
	...
	_expect(screen.choice_list.get_child_count() == 10 and not screen.choice_list.get_child(0).disabled and "ASSEMBLE DECK" in screen.choice_list.get_child(0).text and "0 ICs" in screen.choice_list.get_child(0).text, "canonical catalog leads with the free required assembly action")
	for i in range(1, 10): _expect(screen.choice_list.get_child(i).disabled, "upgrade remains unavailable until assembly")
	...
	_expect(Game.prologue_controller.choose(&"TOOLBOX", &"ASSEMBLE_DECK").success and state.player_state.credits == 25 and state.campaign_state.story_flags.DECK_ASSEMBLED, "required assembly is free and completes the physical deck")
	...
	state.emit_changed()
	room.objects[&"TOOLBOX"].activate()
	_expect(not screen.choice_list.get_child(2).disabled and screen.choice_list.get_child(0).disabled and screen.choice_list.get_child(1).disabled, "affordability and applied status refresh")
	_expect(Game.prologue_controller.choose(&"TOOLBOX", &"DECK_CPU").success and state.player_state.credits == 0, "100 IC upgrade applies")
	_expect(int(state.player_state.hardware.DECK_CPU) == 2 and (room.dressing[&"CPU_UPGRADE"].node as Node3D).visible, "purchased hardware immediately leaves persistent desk evidence")
	_expect(state.campaign_state.story_flags.TOOLBOX_OPEN and (room.dressing[&"DESK_CABLE"].node as Node3D).visible, "persistent toolbox state adds work clutter")
	_expect(controller.execute(&"WINDOW", &"RAISE_BLINDS").success, "main blinds expose a persistent authored state")
	controller.primary(&"WINDOW_TABLE")
	_expect(exterior.window_open, "second window opens audio portal independently")
	controller.primary(&"WINDOW_TABLE")
	_expect(room.display_anchors[1].current_item.id == "DECK_MODULE", "acquired deck fills an authored display anchor")
	state.player_state.inventory.append({"item_id": &"BASIC_NETWORK_ADAPTER", "quantity": 1})
	state.emit_changed()
	_expect(room.display_anchors[0].current_item.id == "BASIC_NETWORK_ADAPTER", "inventory acquisition fills its authored display anchor")
	...
	_expect(returned.exterior.window_open and returned.display_anchors[0].get_child_count() == 1 and returned.display_anchors[1].current_item.id == "DECK_MODULE" and not returned.objects[&"STARTER_DECK_BOX"].visible and returned.objects[&"JACK_IN_INTERFACE"].visible, "window, acquired anchors, box, and computer restore on reload")
	...
	_expect(restored.campaign_state.story_flags.TOOLBOX_OPEN and returned.objects[&"TOOLBOX"].get_node("Lid").rotation.x < -1 and (returned.dressing[&"DESK_CABLE"].node as Node3D).visible, "toolbox and its authored clutter restore on reload")
	_expect(restored.campaign_state.story_flags.WINDOW_BLINDS_RAISED and returned.objects[&"WINDOW"].get_child(6).rotation_degrees.x > 60, "blind state restores on reload")
