...
enum Screen { SPLASH, MAIN_MENU, LOAD, SETTINGS, CREDITS }
...
func _ready() -> void:
	...
	set_save_provider(FrontendSaveProvider.new())
...
func _load_selected_save(save_id: StringName) -> void:
	if save_provider != null: save_provider.continue_save(save_id)
...
func _show_immediately(destination: Screen) -> void:
	for screen in [splash, main_menu, load_screen, settings_screen, credits]: screen.hide(); screen.modulate.a = 1.0
...
func _activate(destination: Screen) -> void:
	...
	elif destination == Screen.LOAD: load_screen.call_deferred("focus_default")
